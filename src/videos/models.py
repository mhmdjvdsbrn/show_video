...
class Video(models.Model):
    ...
    #bulid get_video_id that use playlists model
    def get_video_id(self):
        if not self.is_published:
            return None
        return self.video_id

    #property for simple and beautiful code
    @property
    #if now active ,PUBLISH ,now publish_timestamp not None :publish_timestamp is now
